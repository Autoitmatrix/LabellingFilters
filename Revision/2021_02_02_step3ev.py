...
info_handler = logging.FileHandler(save_dir + "/" + runid + '-EV-info.log')
...
warnings_handler = logging.FileHandler(save_dir + "/" + runid + '-EV-warnings.log')
warnings_logger.addHandler(warnings_handler)

if logger.getEffectiveLevel() == 10:
    debug_handler = logging.FileHandler(save_dir + "/" + runid + '-EV-debug.log')
...
def set_threshold(fil_act, k, percent=False):
    ...
    input:
      ...
      k: number indicating used method for threshold. 0 for layer average as threshold, else k-most or k%-most
    ...
    if k<=0 or (percent and k>100):
        if k<0:
            warn("k too small. The parameter k is set to 0 (average) for all layers")
        if percent and k>100:
            warn("k too big. The parameter k is set to 0 (average) for all layers")
        for i, layer in enumerate(fil_act_t):
            fil_act_t[i]=threshold_average()
    else:
        if percent:
            for i, layer in enumerate(fil_act_t):
                k_per = int(round((k / 100) * len(layer)))
                fil_act_t[i]=threshold_k(k_per)
        else:
            for i, layer in enumerate(fil_act_t):
                if k > len(layer):
                    warn("k too big for layer "+str(i)+". The parameter k is set to 0 (average) for this layer")
                    fil_act_t[i]=threshold_average()
                else:
                    fil_act_t[i]=threshold_k(k)
    return fil_act_t
