# -*- coding: utf-8 -*-
"""Step2FilterlabelsFINAL.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1reYxzuSJEjJ1uKD7ziiWcvKaF-iZvuxx
...
pickle_file="/home/ws/ns1888/FilterLabel/codes/resultstest4/2020-10-26-1938-VGG16-CA-vars"#e.g."2020-10-23-1615-VGG16-CA-vars"
...
if not save_dir:
  save_dir=os.getcwd()
save_vars=save_dir+"/"+runid+"-"+network+"-LF-vars"
save_vars_names=save_dir+"/"+runid+"-"+network+"-LF-vars-names"
save_vars_list=[]
...
info_handler = logging.FileHandler(save_dir+"/"+runid+'-LF-info.log')
...
warnings_handler = logging.FileHandler(save_dir+"/"+runid+'-LF-warnings.log')
...
if logger.getEffectiveLevel()==10:
  debug_handler = logging.FileHandler(savedir+"/"+runid+'-LF-debug.log')
...
logger.info("loaded following variables:"+str(var_names))
logger.debug("variables of step1 ready")
...
def label_it(fil_act_t,label,labels_old=[]):
  ...
  inputs:
  - filt_act_t: thresholded active filters; list with np-arrays for each layer as elements; these np-arrays contain the thresholded activations (0:filter inactive, 1: filter active)
...
for i,el in enumerate(tqdm(k)):
  for j in range(num_classes):
    logger.info("determining most active filters, k="+str(el)+",class "+str(j+1)+" of "+str(num_classes))
    if not fil_act[j]:
      continue
    fil_act_t=set_threshold(fil_act[j],el,False)
...
for i,el in enumerate(tqdm(k)):
  for j in range(num_classes): 
    ...
    if not fil_act[j]:
      continue
    fil_act_t=set_threshold(fil_act[j],el,True)
