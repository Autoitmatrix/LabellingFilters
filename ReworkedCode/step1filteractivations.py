...
network = "VGG16"
images_dir = "/content/ILSVRC2012_img_train"
...
runid = now.strftime("%Y-%m-%d-%H%M")+'-QFA'
if not save_dir:
    save_dir = os.getcwd()
...
""" Get Data and Compute Activations """
...
with open(save_vars_info, "a") as txt:
    txt.write("- train_filenames: list with filenames of train set\n")
    txt.write("- test_filenames: list with filenames of test set\n")
    txt.write("- labels_id: dictionary of numerical train labels and "
...
for i in tqdm(range(num_classes)):
    ...
    class_images = [train[j][0] for j in range(num_images) if train.labels[j] == i]
    ...
    with open(save_vars_names, "wb") as p_2: #overwrites vars list
